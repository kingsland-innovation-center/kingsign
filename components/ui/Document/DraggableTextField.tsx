...
export interface DraggableTextFieldProps {
  id: string;
  text: string;
...
export function DraggableTextField({
  ...
  const handleResize = (
    ...
    if (onResize) {
      onResize(id, Math.round(data.size.width), Math.round(data.size.height));
    }
  };
  ...
  const handleFontSizeChange = (increase: boolean) => (e: React.MouseEvent) => {
    e.stopPropagation();
    if (onFormatChange) {
      const newSize = increase ? fontSize + 2 : Math.max(8, fontSize - 2);
      onFormatChange(id, { fontSize: newSize });
    }
  };
  ...
  // Text style based on formatting options
  const textStyle = {
      ...
      >
        {allowEdit ? (
          ...
          >
            ...
            {/* Text field content */}
            <div
              className="relative w-full h-full flex items-center p-2 overflow-auto"
              onClick={handleEdit}
            >
              <p
                className="black select-none cursor-pointer break-words w-full overflow-y-auto max-h-full"
                style={textStyle}
              >
                {text}
              </p>
